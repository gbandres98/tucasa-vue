...
export default function usePriceCalculations() {
  ...
  const optionPrice = computed(() =>
    options.value.reduce((acc, option) => {
      ...
      if (selectedValue) return acc + selectedValue.price;
      else return acc;
    }, 0)
  );

  const terrainPrice = computed(() =>
    terrain.value ? terrain.value.price : 0
  );
